...
/* Guardia de idempotencia: Stripe reintenta el webhook si respondemos 5xx */
export async function ventaExists(orderId) {
  const token = await getAccessToken()
  const res = await fetch(sheetsUrl('Ventas!A:A'), {
    headers: { Authorization: `Bearer ${token}` },
  })
  if (!res.ok) throw new Error(`Sheets read HTTP ${res.status}: ${await res.text()}`)
  const { values = [] } = await res.json()
  return values.some((r) => r[0] === orderId)
}
