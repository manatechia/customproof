...
/* Columna `destacado`: un número marca el producto y fija su puesto en el Top;
   sí/x/true/ok lo marcan sin puesto (van después de los numerados). Vacío,
   no/false/0 = no destacado. */
function parseDest(value) {
  const v = String(value ?? '').trim()
  if (!v || /^(no|false|0)$/i.test(v)) return { dest: false, destOrden: null }
  const n = parseNum(v)
  if (n !== null) return { dest: true, destOrden: n }
  return { dest: /^(si|sí|s|x|yes|true|ok|destacado)$/i.test(v), destOrden: null }
}
